...
1. 2019-11-11
  - [x] 开发dsp报表优化
  - [x] 联调联盟上传生效功能点
  - 首页分析要重视起来，排查问题。从内到外解决
  - 受众分析崔天龙不弄了，我来做。需要跟产品确认受众分析的排期
  - [git学习](http://ydwiki.yidian-inc.com/pages/viewpage.action?pageId=32177017) —— ZSW
    - 合并代码: git rebase.
    ```
    // 在本地分支执行
    git rebase
...
2. 2019-11-12
  - 昨天双11 给妈妈买了iphone 11, 妈妈果然说我有钱没处花，希望我存起来在北京买房用。“未来的生活是什么样的我不知道，但我知道我妈妈现在需要一个新手机”，说完这话我妈妈就没理我。 给 12 买了一包猫粮比非双11 期间便宜了200多，可惜不想囤货，就没再买了
  - [x] 联盟上传外部数据 —— 生效部分联调完成, 已提测。经 风浪中船 提醒下载部分在不同浏览器表现不一致. 分别是一点内部的文件服务器、CDN, 经强哥提示, 或许是文件返回的 content-type 不一致。可以
  ```
  ...
  ```
  - [x] 开发 dsp 报表优化 - 联盟接入
  - [x] 数据平台-客户健康度异常, 验证与前端无关紧要, 与后端相关
  ```
...
3. 2019-11-13 
  - [ ] DSP 新增 dspName, 数据平台暂时不敢上线，怕出现
  - [ ] 隐藏 adMaster-基础数据 tab 页 
  - [x] 百川的 bugList 清单
...
4. 2019-11-14
  ...
  - [x] 数据平台上线
    ...
    - 第三方监控 消息中心 CTL 擦屁股
  - 【领取抽纸】商业化产品技术部最新人员名单 已发送，现在团队一共33人
  - [ ] 
